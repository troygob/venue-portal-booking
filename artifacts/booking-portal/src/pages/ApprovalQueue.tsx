  ...
  async function load() {
    ...
    if (list.length > 0) {
      const { data: steps } = await supabase
        ...
        .in('proposal_id', list.map((p: any) => p.proposal_id))
        .eq('status', 'Approved');
      const map: Record<string, string[]> = {};
      for (const s of steps ?? []) {
        const proposal = list.find((p: any) => p.proposal_id === s.proposal_id);
    ...
    <div>
      <header className="mb-6">
        ...
        <p className="text-muted text-sm mt-1.5 max-w-2xl">
          ...
          but a proposal is only fully approved once all three have signed off.
        </p>
      </header>
