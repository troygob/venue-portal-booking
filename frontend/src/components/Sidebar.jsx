...
export default function Sidebar() {
  ...
  const content = (
    <>
      ...
      {isDemo && (
        <div className="bg-gold/15 border border-gold/40 rounded-lg px-3.5 py-3 mb-4">
          ...
          <select
            ...
            className="w-full text-xs rounded-md px-2 py-1.5 bg-white text-navy-deep font-semibold border border-transparent focus-visible:ring-2 focus-visible:ring-gold"
          >
            {DEMO_ROLES.map((r) => (
      ...
      <div className="bg-white/[0.07] rounded-lg px-3.5 py-3 mb-5">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            <div className="text-sm font-semibold truncate">{profile?.first_name ? `${profile.first_name} ${profile.last_name}` : profile?.email}</div>
            {/* Role is display-only — it was assigned automatically from the
                sign-up email and cannot be changed here (except in demo mode). */}
            <div className="text-white/65 text-xs truncate">{ROLE_LABELS[role]}</div>
          </div>
        </div>
      </div>
      ...
      <button
        onClick={signOut}
        className="flex items-center gap-2 text-white/65 text-sm text-left pt-5 mt-auto hover:text-white rounded-md"
      >
